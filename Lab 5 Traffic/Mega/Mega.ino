...
/* SCHEDULER FLAGS */
// index for flags (0 -> 8) : measure, compute, display, annunciate, status, keypad, EKG Capture, EKG Process, Remote, traffic
unsigned short addFlags[10] = {1, 1, 1, 0, 0, 1, 0, 0, 0, 0};
unsigned short removeFlags[10] = {0};
...
TrafficTaskData dataForTraffic;
...
void EKGCaptureDataFunc(void* data) {
    ...
    for (int i = 0; i < EKG_SAMPLES; i++) {
      *(dataStruct.EKGRawBufPtr + i) = 32*sin(2 * 3.14 * f * i/fs);
...
void EKGProcessDataFunc(void* data) {
    ...
    int vReal[EKG_SAMPLES];
    int vImag[EKG_SAMPLES];
    ...
    EKGFreqBufTemp[0] = (int) (peak*(fs/256));
...
void displayDataFunc(void* x) {
    ...
    menuButtons[2].initButton(&tft, 125, 195, 180, 50, ILI9341_WHITE, ILI9341_BLUE, ILI9341_WHITE, "EMERGENCY", 2);
            ...
            for (uint8_t b = 0; b < 4; b++) {
                if (menuButtons[b].justPressed()) {
                    if(b == 3)  { // Menu
                        ...
                        addFlags[7] = 1;
                        break;
                    } else if (b == 1) { // emergency
                        emergency = true;
                        tft.setCursor(0,0);
                        tft.fillScreen(BLACK);
                        addFlags[9] = 1;
                        removeFlags[8] = 1; //remove remote display
                         measurementSelection = 5;
                        // TODO: CORRECT THESE FLAGS, need to include EKG
                        addFlags[0] = 1;
                        addFlags[1] = 1;
...
void pointData() {
  // Point data in data structs to correct information
    ...
    dataForComputeTMP.respirationRateRawPtr = respirationRateRawBuf;
    ...
    // Clear Traffic
    TrafficTaskData dataForTrafficTMP;
    dataForTrafficTMP.patientNamePtr = &patientName;
    dataForTraffic = dataForTrafficTMP;
    ...
    // Traffic
    TCB TrafficTaskTMP;
    TrafficTaskTMP.taskFuncPtr = &emergencyFunc;
    TrafficTaskTMP.taskDataPtr = &dataForTraffic;
    TrafficTaskTMP.next = NULL;
    TrafficTaskTMP.prev = NULL;
    TrafficTask = TrafficTaskTMP; 
...
void scheduler() {
    if(localDisplayEnabled) {
        ...
        for(int i = 0; i < 10; i++) { // Add all the tasks
            if(addFlags[i]) {
        ...
        for(int i = 0; i < 10; i++) { // checks remove task flags
            if(removeFlags[i]) {
...
void serialEvent() {
    ...
    if(!firstRunComplete && 'I' != previousChar) {
        ...
        serialInMode = 0;
    } else if ('A' == previousChar && emergency) {
      String address = "";
      Serial.print("\nAddress: ");
...
void emergencyFunc(void* x) {
    TrafficTaskData* data = (TrafficTaskData*)x;
    TrafficTaskData dataStruct = *data;
    ...
    Serial.println("\e[31mEMERGENGY\e[m");
    Serial.println("\e[31mEMERGENGY\e[m");
    Serial.print("\e[4mPatient\e[m: "); Serial.println(*(dataStruct.patientNamePtr));
    Serial.println("\e[31mEMERGENGY\e[m");
    Serial.println("\e[4m Press A to enter address for the ambulance\e[m");
}
