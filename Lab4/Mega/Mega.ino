...
/* SCHEDULER FLAGS */
// index for flags (0 -> 6) : measure, compute, display, annunciate, status, keypad
...
unsigned short functionSelect = 0,
               measurementSelection = 4,// try putting this zero for the defualt value
               alarmAcknowledge = 0;
...
void parseMessage() {
  ...
  while (Serial1.available() > 0) {
    ...
    if ('>' == currChar) {
      ...
      // Serial.println(requestingTaskID);
      delay(20);
      requestedFunction = Serial1.read();
      Serial1.read();
      // Serial.println(requestedFunction);
      delay(20);
...
//  Serial.println("abc");
//  Serial.println(respirationRateRawBufTemp[0]);
//  Serial.println(respirationRateRawBufTemp[1]);
  // Update new values
...
//  Serial.println(*dataStruct.respirationRateRawPtr);
...
//If we keep the println for the *dataStruct.respirationRateRawPtr, the printed value for respiration rate is 184, but it is 7 without the println line
//  Serial.println("Compute pointer");
//  Serial.println(*dataStruct.respirationRateRawPtr);
//  Serial.println(rawResp);
...
void annunciateDataFunc(void* x) {
  // measurementSelection = 3; // Means get all values
  ...
  if ('A' == mode) { // Annunciate
    ...
    while (keepSensing && unoCounter - tempCounter <= 2) {
      ...
      if (backButton[0].justPressed()) {
        ...
        mode = 'B';
        measurementSelection = 5; // do nothing this time
...
void menuView() {
  ...
  measureSelectButtons[3].initButton(&tft, 125, 195, 180, 35, ILI9341_WHITE, ILI9341_BLUE, ILI9341_WHITE, "Respiration", 2);
